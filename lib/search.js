...
import { getInsightsArticles } from './insights';
...
export async function searchSite(query) {
  const q = (query || '').trim();
  if (!q) return { destinations: [], cities: [], guides: [], glossary: [], insights: [] };

  const results = { destinations: [], cities: [], guides: [], glossary: [], insights: [] };
  ...
  try {
    ...
    results.insights = articles
      .filter((a) => matches(q, a.title, a.subtitle, a.excerpt, a.category))
      .map((a) => ({
        title: a.title,
        url: `/insights/${a.slug}`,
        snippet: a.excerpt || a.category || '',
      }));
  } catch (err) {
