...
import { getGlossaryTerms, autoLinkGlossaryTerms } from './glossary';
...
function mapDestination(page, type) {
  ...
  return {
    ...
    insuranceAffiliateExcluded: text(p['Insurance Affiliate Excluded']),
    ...
    usTaxTreaty: text(p['US Tax Treaty']),
    doubleTaxationNotes: text(p['Double Taxation Notes']),
    ...
    budgetDefaults: {
      ...
      lastVerified: text(p['Budget Defaults Last Verified']),
    },
  };
}

// Raw Notion queries, wrapped in unstable_cache below. Traffic to pages that
// must stay dynamic per-visitor (compare, match, recommend, search — they
// read searchParams, so Next can't statically cache the page itself) still
// funnels through these same functions, so caching here is what actually
// protects Notion's ~3 req/sec rate limit from a traffic spike on those
// pages. Destination/calculator/guide pages already cache at the page level
// via `revalidate`, so this is a second, cheap layer for those, not a
// requirement.
async function fetchCountriesFromNotion() {
...
const getCachedCountries = unstable_cache(fetchCountriesFromNotion, ['notion-countries'], {
  revalidate: 3600,
  tags: ['notion-countries'],
});
...
async function fetchStatesFromNotion() {
  const client = getClient();
...
const getCachedStates = unstable_cache(fetchStatesFromNotion, ['notion-states'], {
  revalidate: 3600,
  tags: ['notion-states'],
});
...
// Domains we have an active affiliate/referral relationship with. Links to
// these get rel="noopener sponsored" (Google's required signal for paid
// links) instead of the default rel="noreferrer" used for plain citations.
const SPONSORED_DOMAINS = ['safetywing.com', 'wise.com', 'tpx.li', 'prf.hn'];

function isSponsoredLink(href) {
  try {
    const { hostname } = new URL(href);
    return SPONSORED_DOMAINS.some((d) => hostname === d || hostname.endsWith(`.${d}`));
  } catch {
    return false;
  }
}

// Affiliate/CTA links referenced by key, never by raw URL in Notion content.
// Notion auto-linkifies any bare URL it sees regardless of surrounding
// syntax, which splits a single rich-text run into multiple fragments and
// breaks any marker-based approach that embeds the URL directly. Keying by
// a short slug avoids ever putting a URL string in Notion content at all.
const CTA_LINKS = {
  wise: 'https://wise.prf.hn/click/camref:1101l5Q7Bc',
  'safetywing-complete':
    'https://safetywing.com/nomad-insurance?selectedPlan=NOMAD_INSURANCE_COMPLETE&referenceID=26572558&campaign=next-horizon-website&utm_campaign=next-horizon-website&utm_source=26572558&utm_medium=Ambassador',
  'safetywing-essential':
    'https://safetywing.com/nomad-insurance?referenceID=26572558&campaign=next-horizon-website&utm_campaign=next-horizon-website&utm_source=26572558&utm_medium=Ambassador',
  'airalo-esim': 'https://airalo.tpx.li/QJuDV0bd',
  ici: 'https://www.internationalinsurance.com/quote/global-health-plans/?utm_medium=cpl&utm_source=nexthorizon&a_bid=dedbdc12',
};

// Custom CTA marker, e.g. [[cta:Get Nomad Health Coverage|safetywing-complete]],
// authored as literal plain text in Notion content. Uses a short key (never
// a raw URL) specifically so Notion's automatic bare-URL link detection has
// nothing to latch onto and split the surrounding text into fragments.
const CTA_MARKER_RE = /\{\{cta:([^}|]+?)\|([a-z0-9-]+)\}\}/g;
  ...
  return richTextArray
    .map((rt) => {
      ...
      if (raw.includes('{{cta:')) {
        ...
        let lastIndex = 0;
        let match;
        CTA_MARKER_RE.lastIndex = 0;
        while ((match = CTA_MARKER_RE.exec(raw)) !== null) {
          result += escapeHtmlText(raw.slice(lastIndex, match.index));
          const label = escapeHtmlText(match[1]);
          const href = CTA_LINKS[match[2]];
          if (href) {
            const cls = sponsoredLinkIndex === 0 ? 'ctaPrimary' : 'ctaSecondary';
            sponsoredLinkIndex += 1;
            const rel = isSponsoredLink(href) ? 'noopener sponsored' : 'noreferrer';
            result += `<a class="${cls}" href="${href}" target="_blank" rel="${rel}">${label}</a>`;
          } else {
            // Unknown key — fail visibly in the CMS rather than silently
            // dropping the link, so a typo gets caught during review.
            result += `[unknown CTA key: ${escapeHtmlText(match[2])}]`;
          }
          lastIndex = CTA_MARKER_RE.lastIndex;
        }
        result += escapeHtmlText(raw.slice(lastIndex));
        if (rt.annotations?.code) result = `<code>${result}</code>`;
        if (rt.annotations?.bold) result = `<strong>${result}</strong>`;
        if (rt.annotations?.italic) result = `<em>${result}</em>`;
        return result;
      }
      ...
      if (rt.href) {
        ...
        if (resolved.keep) {
          ...
          if (!isInternal && isSponsoredLink(resolved.href)) {
            const cls = sponsoredLinkIndex === 0 ? 'ctaPrimary' : 'ctaSecondary';
            sponsoredLinkIndex += 1;
            s = `<a class="${cls}" href="${resolved.href}" target="_blank" rel="noopener sponsored">${s}</a>`;
          } else {
            s = isInternal
...
// Marks an optional "## SEO Metadata" heading inside a guide sub-page.
// Everything from this heading onward is excluded from visitor-facing HTML
// and instead parsed for "SEO Title: ..." / "SEO Description: ..." override
// lines — lets us tune search-result titles/descriptions without touching
// the on-page H1 (which would change the URL slug via slugify) or the
// visible article content.
function isSeoMetadataHeading(block) {
  const headingText =
  ...
  return /^seo metadata$/i.test(headingText.trim());
}
...
async function renderBlocksToHtml(client, blocks) {
  ...
  for (const block of blocks) {
    ...
    if (t === 'bulleted_list_item' || t === 'numbered_list_item' || t === 'to_do') {
      const wantType = t === 'numbered_list_item' ? 'ol' : 'ul';
      if (listType && listType !== wantType) flushList();
      listType = wantType;
      if (t === 'to_do') {
        // Notion checklist items — rendered as a static (non-interactive)
        // checklist glyph since this is server-rendered visitor content,
        // not an editable to-do list.
        const checked = block.to_do?.checked;
        listBuffer.push(`<li>${checked ? '☑' : '☐'} ${richTextToHtml(block.to_do?.rich_text)}</li>`);
      } else {
    ...
    if (t === 'heading_1') {
      ...
      const colorClass =
        color === 'yellow_background' ? ' calloutYellow' :
        color === 'blue_background' ? ' calloutBlue' :
        color === 'green_background' ? ' calloutCta' : '';
      const bodyHtml = richTextToHtml(block.callout?.rich_text);
      // Any callout whose body renders a CTA link (via a {{cta:...}} marker
      // or a sponsored-domain link) is an affiliate placement. Append the
      // "Affiliate Link" disclosure tag automatically so editors never have
      // to remember to type it by hand in Notion — one place to keep this
      // consistent and accurate across every affiliate callout on the site.
      const isAffiliateCallout = /class="cta(Primary|Secondary)"/.test(bodyHtml);
      const tagHtml = isAffiliateCallout ? '<span class="calloutTag">Affiliate Link</span>' : '';
      html += `<div class="callout${colorClass}">${emoji ? `<span>${emoji}</span> ` : ''}${bodyHtml}${tagHtml}</div>`;
    }
 ...
 */
// Exposes a CTA_LINKS URL by key for use outside the Notion rich-text
// pipeline — e.g. a hardcoded React affiliate card that needs the same
// tracked link without going through richTextToHtml.
export function getCtaLink(key) {
...
export async function getPageContentHtml(pageId) {
  ...
  try {
    ...
    const html = await renderBlocksToHtml(client, blocks);
    // Auto-link the first mention of any glossary term to its definition.
    // Runs here (the single choke point every guide/destination/city page
    // content flows through) so new glossary terms link everywhere
    // automatically without editing individual Notion pages.
    const terms = await getGlossaryTerms();
    return autoLinkGlossaryTerms(html, terms);
  } catch (err) {
...
const CITIES_REGIONS_DB_ID = '6886765d-7678-460d-9130-09b33bc5f5fc';
 ...
 */
async function fetchRawCitiesRegionsFromNotion() {
  const client = getClient();
  if (!client) return null;
...
const getCachedRawCitiesRegions = unstable_cache(
  fetchRawCitiesRegionsFromNotion,
  ['notion-cities-regions'],
  { revalidate: 3600, tags: ['notion-cities-regions'] }
);

async function getRawCitiesRegions() {
  return getCachedRawCitiesRegions();
}
...
export async function getCitiesRegions() {
  ...
  (countries || []).forEach((c) => parentById.set(c.id, {
    slug: c.slug,
    name: c.name,
    type: 'country',
    insuranceAffiliateExcluded: c.insuranceAffiliateExcluded,
  }));
  (states || []).forEach((s) => parentById.set(s.id, {
    slug: s.slug,
    name: s.name,
    type: 'state',
    insuranceAffiliateExcluded: s.insuranceAffiliateExcluded,
  }));
  ...
  return rawCities
    .map((city) => {
      ...
      return {
        ...
        // Cities have no exclusion flag of their own — they inherit their
        // parent country/state's Insurance Affiliate Excluded setting, so a
        // Prague city profile stays excluded just like the Czech Republic
        // country page is.
        insuranceAffiliateExcluded: Boolean(parent.insuranceAffiliateExcluded),
      };
    })
    .filter(Boolean);
}
...
const getCachedGuideHub = unstable_cache(
  async (pageId) => fetchGuideHubFromNotion(pageId),
  ['notion-guide-hub'],
  { revalidate: 3600, tags: ['notion-guide-hub'] }
);

export async function getGuideHub(pageId) {
  if (!pageId) return null;
  return getCachedGuideHub(pageId);
}

async function fetchGuideHubFromNotion(pageId) {
  const client = getClient();
...
export async function getGuideSubPageBySlug(hubId, slug) {
  ...
  const client = getClient();
  if (!client) return { title: topic.title, html: '' };

  const blocks = await getBlockChildren(client, topic.id);

  let seoTitle = null;
  let seoDescription = null;
  const seoMarkerIndex = blocks.findIndex((b) => isSeoMetadataHeading(b));
  const contentBlocks = seoMarkerIndex === -1 ? blocks : blocks.slice(0, seoMarkerIndex);

  if (seoMarkerIndex !== -1) {
    for (const b of blocks.slice(seoMarkerIndex + 1)) {
  ...
  const rawHtml = await renderBlocksToHtml(client, contentBlocks);
  const terms = await getGlossaryTerms();
  const html = autoLinkGlossaryTerms(rawHtml, terms);

  return { title: topic.title, html, seoTitle, seoDescription };
}
