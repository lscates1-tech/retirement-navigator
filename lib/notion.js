...
// Custom CTA marker, e.g. [[cta:Get Nomad Health Coverage->https://example.com]],
// authored as literal plain text in Notion content (never through Notion's
// own [text](url) markdown-link parsing, which has proven unreliable for
// affiliate links — it sometimes produces a working href but leaves the raw
// markdown source as the visible label). Parsed and rendered entirely here,
// so nothing depends on Notion's link-conversion behavior.
const CTA_MARKER_RE = /\[\[cta:([^\]|]+?)->(https?:\/\/[^\]]+?)\]\]/g;
    ...
    .map((rt) => {
      ...
      if (raw.includes('[[cta:')) {
        ...
        while ((match = CTA_MARKER_RE.exec(raw)) !== null) {
          ...
          const rel = isSponsoredLink(match[2]) ? 'noopener sponsored' : 'noreferrer';
