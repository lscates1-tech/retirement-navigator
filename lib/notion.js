    ...
    .map((rt) => {
      ...
      if (rt.href) {
        ...
        if (resolved.keep) {
          ...
          const rel = isSponsoredLink(resolved.href) ? 'noopener sponsored' : 'noreferrer';
          s = isInternal
            ? `<a href="${resolved.href}">${s}</a>`
            : `<a href="${resolved.href}" target="_blank" rel="${rel}">${s}</a>`;
        }
...
// Same as richTextToHtml, but for callouts flagged as CTA callouts (green
// background): the first link renders as a solid primary button, any link
// after it renders as a smaller secondary text link. Sponsored-domain
// detection and internal-link handling are shared with richTextToHtml.
function richTextToHtmlCta(richTextArray) {
  if (!richTextArray || !richTextArray.length) return '';
  let linkIndex = 0;
  return richTextArray
    .map((rt) => {
      ...
      if (rt.href) {
        ...
        if (resolved.keep) {
          const isInternal = resolved.href.startsWith('/');
          const rel = isSponsoredLink(resolved.href) ? 'noopener sponsored' : 'noreferrer';
          const cls = linkIndex === 0 ? 'ctaPrimary' : 'ctaSecondary';
          linkIndex += 1;
          s = isInternal
            ? `<a class="${cls}" href="${resolved.href}">${s}</a>`
            : `<a class="${cls}" href="${resolved.href}" target="_blank" rel="${rel}">${s}</a>`;
        }
      }
      return s;
    })
...
async function renderBlocksToHtml(client, blocks) {
  ...
  for (const block of blocks) {
    ...
    if (t === 'heading_1') {
      ...
      const isCta = color === 'green_background';
      const colorClass =
        color === 'yellow_background' ? ' calloutYellow' :
        color === 'blue_background' ? ' calloutBlue' :
        isCta ? ' calloutCta' : '';
      const bodyHtml = isCta
        ? richTextToHtmlCta(block.callout?.rich_text)
        : richTextToHtml(block.callout?.rich_text);
      html += `<div class="callout${colorClass}">${emoji ? `<span>${emoji}</span> ` : ''}${bodyHtml}</div>`;
    }
