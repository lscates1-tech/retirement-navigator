 ...
 * framing) from its list of linked sub-pages (child_page blocks), since the
 * sub-pages themselves aren't built as site pages yet. Returns
 * { html, topics: [{ title }] }. Returns null if unconfigured or not found.
 */
  ...
  try {
    ...
    for (const block of blocks) {
      if (block.type === 'child_page') {
        topics.push({ title: block.child_page?.title || 'Untitled' });
