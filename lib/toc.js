...
/**
 * Simpler alternative to buildSectionedArticle: adds an id to each <h2>
 * in place, without wrapping sections in collapsible <details>. Used for
 * content meant to be read straight through (like Insights articles),
 * where collapsing sections would be the wrong interaction model.
 */
export function addHeadingAnchors(html) {
  ...
  const modified = html.replace(/<h2>(.*?)<\/h2>/g, (match, inner) => {
    const plainText = decodeEntities(inner.replace(/<[^>]+>/g, '').trim());
