...
// ---------------------------------------------------------------------------
// ITALY -- Standard progressive IRPEF vs. the flat 7% Southern Incentive
// (Art. 24-ter TUIR) for qualifying municipalities under 30,000 residents
// ---------------------------------------------------------------------------

// 2026 IRPEF national brackets (Law No. 199 of 30 Dec 2025 cut the middle
// bracket from 35% to 33%). Sources disagree on whether the 33% band's upper
// threshold is EUR 50,000 or EUR 60,000 for the 2026 tax year -- most
// corroborate EUR 50,000, which is what's used here. Re-verify each year;
// this is exactly the kind of figure that shifts with each Budget Law.
...
// Regional (addizionale regionale, 0.7-3.33%) and municipal (addizionale
// comunale, up to 0.9%) surtaxes vary by comune and can't be modeled per-city
// here. This is a representative blended rate (~roughly the middle of the
// national range) applied flat to taxable income -- treat as directional,
// not a substitute for checking the specific region/comune.
const ITALY_REPRESENTATIVE_SURTAX_RATE = 0.0253;

const ITALY_FLAT_RATE = 0.07;

export const ITALY_TAX_REGIONS = {
  standard: {
    label: 'Standard Zone (Rome, Milan, Florence)',
    description: 'Standard progressive IRPEF (23%-43%) plus regional and municipal surtaxes.',
  },
  flat_7: {
    label: '7% Southern Incentive Zone (Qualifying towns <30,000 population)',
    description:
      'Flat 7% substitute tax on foreign pension/income under Art. 24-ter TUIR, for up to 10 years. Replaces IRPEF and regional/municipal surtaxes entirely -- both are set to 0 under this regime.',
  },
};

/**
 * Calculate Italy income tax for one tax year: standard progressive IRPEF +
 * representative regional/municipal surtax, or the flat 7% Southern
 * Incentive substitute tax (Art. 24-ter TUIR).
 *
 * @param {object} p
 * @param {'standard'|'flat_7'} p.taxRegion
 * @param {number} p.taxableIncome - annual income, EUR
 */
export function calculateItalyTax({ taxRegion, taxableIncome }) {
  if (!ITALY_TAX_REGIONS[taxRegion]) {
    throw new Error(`Unknown Italy taxRegion "${taxRegion}". Expected one of: ${Object.keys(ITALY_TAX_REGIONS).join(', ')}`);
  }

  function taxForZone(zone) {
    if (zone === 'flat_7') {
      return { nationalTax: round2(taxableIncome * ITALY_FLAT_RATE), regionalTax: 0 };
    }
    return {
      nationalTax: round2(applyBrackets(taxableIncome, ITALY_IRPEF_BRACKETS)),
      regionalTax: round2(taxableIncome * ITALY_REPRESENTATIVE_SURTAX_RATE),
    };
  }

  const { nationalTax, regionalTax } = taxForZone(taxRegion);
  const standard = taxForZone('standard');
  const thisTotal = nationalTax + regionalTax;
  const standardTotal = standard.nationalTax + standard.regionalTax;
  const totalSavingsComparedToHighestZone = round2(standardTotal - thisTotal);

  return {
    country: 'Italy',
    taxRegion,
    regionLabel: ITALY_TAX_REGIONS[taxRegion].label,
    currency: 'EUR',
    totalTaxDrag: round2(nationalTax + regionalTax),
    breakdown: [nationalTax, regionalTax, totalSavingsComparedToHighestZone],
    breakdownDetail: { nationalTax, regionalTax, totalSavingsComparedToHighestZone },
    notes: [
      'Regional/municipal surtax uses a representative blended rate, not a comune-specific figure -- actual surtax varies by region and municipality.',
      taxRegion === 'flat_7'
        ? 'The 7% regime requires not having been an Italian tax resident in the prior 5 years and moving to a qualifying municipality -- eligibility isn\'t automatic.'
        : null,
    ].filter(Boolean),
  };
}
...
// ---------------------------------------------------------------------------
// GREECE -- Standard progressive income tax vs. the flat 7% foreign
// pensioner regime (Article 5B of the Greek Income Tax Code)
// ---------------------------------------------------------------------------

// 2026 reform (Law 5246/2025) brackets -- cut most mid brackets by 2 points
// and moved the 44% top rate to start at EUR 60,000 (previously EUR 40,000).
const GREECE_INCOME_BRACKETS = [
...
const GREECE_FLAT_RATE = 0.07;

export const GREECE_TAX_REGIONS = {
  standard: {
    label: 'Standard Tax Resident',
    description: 'Standard progressive Greek income tax (9%-44%), 2026 brackets under Law 5246/2025.',
  },
  art_5b: {
    label: '7% Foreign Pensioner Regime (Art. 5B)',
    description:
      'Flat 7% tax on foreign-sourced income for up to 15 years, for qualifying foreign pensioners who transfer tax residence to Greece.',
  },
};

/**
 * Calculate Greece income tax for one tax year: standard progressive scale,
 * or the flat 7% Article 5B foreign pensioner regime. Greece has no
 * regional/municipal income surtax equivalent to Italy's, so regionalTax is
 * always 0 here -- this is a national-only tax.
 *
 * @param {object} p
 * @param {'standard'|'art_5b'} p.taxRegion
 * @param {number} p.taxableIncome - annual income, EUR
 */
export function calculateGreeceTax({ taxRegion, taxableIncome }) {
  if (!GREECE_TAX_REGIONS[taxRegion]) {
    throw new Error(`Unknown Greece taxRegion "${taxRegion}". Expected one of: ${Object.keys(GREECE_TAX_REGIONS).join(', ')}`);
  }

  function nationalTaxForZone(zone) {
    if (zone === 'art_5b') return round2(taxableIncome * GREECE_FLAT_RATE);
    return round2(applyBrackets(taxableIncome, GREECE_INCOME_BRACKETS));
  }

  const nationalTax = nationalTaxForZone(taxRegion);
  const standardTax = nationalTaxForZone('standard');
  const totalSavingsComparedToHighestZone = round2(standardTax - nationalTax);

  return {
    country: 'Greece',
    taxRegion,
    regionLabel: GREECE_TAX_REGIONS[taxRegion].label,
    currency: 'EUR',
    totalTaxDrag: nationalTax,
    breakdown: [nationalTax, 0, totalSavingsComparedToHighestZone],
    breakdownDetail: { nationalTax, regionalTax: 0, totalSavingsComparedToHighestZone },
    notes: [
      'Greece has no regional/municipal income-tax surtax equivalent to Italy\'s, so this is a national-only figure.',
      taxRegion === 'art_5b'
        ? 'Art. 5B requires not having been a Greek tax resident for 5 of the prior 6 years and applies only to foreign-sourced income -- eligibility isn\'t automatic.'
        : null,
    ].filter(Boolean),
  };
}
...
export function calculateRegionalTax(params) {
  ...
  if (country === 'Italy') return calculateItalyTax(params);
  if (country === 'Greece') return calculateGreeceTax(params);
  throw new Error(
    `calculateRegionalTax: unsupported country "${country}". Currently supported: "Spain", "United States", "Italy", "Greece". ` +
...
export const TAX_REGIONS_BY_COUNTRY = {
  ...
  Italy: ITALY_TAX_REGIONS,
  Greece: GREECE_TAX_REGIONS,
};
